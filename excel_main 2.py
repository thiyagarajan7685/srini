...
def table_extraction_regex(file_path, table_no, row_nums, col_nums):
    ...
    def get_cell_value(row, col):
        try:
            ...
            logger.info(f"Extracted value at (row={row+1}, col={col+1}): {value}")
            return str(value)
        except IndexError:
            logger.warning(f"Invalid index at (row={row+1}, col={col+1}) — skipping")
            return ""

    result = [get_cell_value(row, col) for row in row_indices for col in col_indices]
    return "\n".join(result)
...
def process_config_rows(config_df: pd.DataFrame, pdf_map: dict, text_extract: bool) -> pd.DataFrame:
    ...
    for _, row in config_df.iterrows():
        ...
        try:
            if pd.notna(table_no) and pd.notna(row_num) and pd.notna(col_num) and not excel_regex_pattern:
                extracted_value = table_extraction_regex(file_path, table_no, row_num, col_num)
...
def main():
    ...
    parser.add_argument("-t", "--text", help="Extract text and save it as a file when set to true.")
    ...
    parser.add_argument("--debug", "--dg", dest="debug", action="store_true", help="Enable debug mode and save logs to a file.")
